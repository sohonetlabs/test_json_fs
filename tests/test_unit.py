...
class TestConstructorValidation:
    ...
    def test_invalid_rate_limit(self):
        ...
        with pytest.raises(
            ValueError, match="rate_limit must be a non-negative number"
        ):
    ...
    def test_invalid_pre_generated_blocks(self):
        ...
        # Zero should fail
        with pytest.raises(
            ValueError, match="pre_generated_blocks must be a positive integer"
        ):
...
class TestJSONFileSystem:
    ...
    def simple_fs(self):
        ...
        return JSONFileSystem(
            json_data, report=False, pre_generated_blocks=10, block_size=1024
        )
